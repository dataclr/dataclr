...
from dataclr.methods import ANOVA
...
def test_ranked_features(dataset, model, metric):
    ...
    anova = ANOVA(model=model, metric=metric, n_results=3, seed=42)
    anova.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(anova.ranked_features_, pd.Series)
    assert not anova.ranked_features_.empty
    assert anova.ranked_features_.dtype.kind in {"i", "f"}
    assert anova.ranked_features_.is_monotonic_increasing
...
def test_results_list(dataset, model, metric):
    X_train, X_test, y_train, y_test = generate_dataset(dataset)
    anova = ANOVA(model=model, metric=metric, n_results=3, seed=42)
    results = anova.fit_transform(X_train, X_test, y_train, y_test)
...
def test_empty_dataset():
    anova = ANOVA(model=LinearRegression(), metric="rmse", n_results=3, seed=42)
    results = anova.fit_transform(

...
from dataclr.methods import Skewness
...
def test_ranked_features(generate_dataset, dataset, model, metric):
    ...
    skewness = Skewness(model=model, metric=metric, n_results=3, seed=42)
    skewness.fit_transform(X_train, X_test, y_train, y_test)
...
def test_results_list(generate_dataset, dataset, model, metric):
    X_train, X_test, y_train, y_test = generate_dataset(dataset)
    skewness = Skewness(model=model, metric=metric, n_results=3, seed=42)
    results = skewness.fit_transform(X_train, X_test, y_train, y_test)
...
def test_empty_dataset():
    skewness = Skewness(model=LinearRegression(), metric="rmse", n_results=3, seed=42)
    results = skewness.fit_transform(

...
from dataclr.methods import SpearmanCorrelation
...
def test_ranked_features(generate_dataset, dataset, model, metric):
    ...
    spearman_correlation.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(spearman_correlation.ranked_features_, pd.Series)
    assert not spearman_correlation.ranked_features_.empty
    assert spearman_correlation.ranked_features_.dtype.kind in {"i", "f"}
    assert spearman_correlation.ranked_features_.is_monotonic_increasing
...
def test_results_list(generate_dataset, dataset, model, metric):
    ...
    results = spearman_correlation.fit_transform(X_train, X_test, y_train, y_test)
...
def test_empty_dataset():
    ...
    results = spearman_correlation.fit_transform(

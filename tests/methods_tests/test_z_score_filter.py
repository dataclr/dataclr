...
from dataclr.methods import ZScore
...
def test_ranked_features(dataset, model, metric):
    ...
    z_score.fit_transform(X_train, X_test, y_train, y_test)
...
def test_empty_dataset():
    z_score = ZScore(model=LinearRegression(), metric="rmse", n_results=3, seed=42)
    results = z_score.fit_transform(

...
from dataclr.methods import MeanAbsoluteDeviation
...
def test_ranked_features(dataset, model, metric):
    ...
    mad = MeanAbsoluteDeviation(model=model, metric=metric, n_results=3, seed=42)
    mad.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(mad.ranked_features_, pd.Series)
    assert not mad.ranked_features_.empty
    assert mad.ranked_features_.dtype.kind in {"i", "f"}
    assert mad.ranked_features_.is_monotonic_increasing
...
def test_results_list(dataset, model, metric):
    X_train, X_test, y_train, y_test = generate_dataset(dataset)
    mad = MeanAbsoluteDeviation(model=model, metric=metric, n_results=3, seed=42)
    results = mad.fit_transform(X_train, X_test, y_train, y_test)

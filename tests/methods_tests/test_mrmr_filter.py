...
from dataclr.methods import mRMR
...
def test_ranked_features(generate_dataset, dataset, model, metric):
    ...
    mrmr = mRMR(model=model, metric=metric, n_results=3, seed=42)
    mrmr.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(mrmr.ranked_features_, pd.Series)
    assert not mrmr.ranked_features_.empty
    assert mrmr.ranked_features_.dtype.kind in {"i", "f"}
    assert mrmr.ranked_features_.is_monotonic_increasing
...
def test_results_list(generate_dataset, dataset, model, metric):
    X_train, X_test, y_train, y_test = generate_dataset(dataset)
    mrmr = mRMR(model=model, metric=metric, n_results=3, seed=42)
    results = mrmr.fit_transform(X_train, X_test, y_train, y_test)
...
def test_empty_dataset():
    mrmr = mRMR(model=LinearRegression(), metric="rmse", n_results=3, seed=42)
    results = mrmr.fit_transform(

...
from dataclr.methods import Entropy
...
def test_ranked_features(generate_dataset, dataset, model, metric):
    ...
    entropy = Entropy(model=model, metric=metric, n_results=3, seed=42)
    entropy.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(entropy.ranked_features_, pd.Series)
    assert not entropy.ranked_features_.empty
    assert entropy.ranked_features_.dtype.kind in {"i", "f"}
    assert entropy.ranked_features_.is_monotonic_increasing
...
def test_results_list(generate_dataset, dataset, model, metric):
    X_train, X_test, y_train, y_test = generate_dataset(dataset)
    entropy = Entropy(model=model, metric=metric, n_results=3, seed=42)
    results = entropy.fit_transform(X_train, X_test, y_train, y_test)
...
def test_empty_dataset():
    entropy = Entropy(model=LinearRegression(), metric="rmse", n_results=3, seed=42)
    results = entropy.fit_transform(

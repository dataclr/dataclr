...
from dataclr.methods import MaximalInformationCoefficient
...
def test_ranked_features(generate_dataset, dataset, model, metric):
    ...
    mic.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(mic.ranked_features_, pd.Series)
    assert not mic.ranked_features_.empty
    assert mic.ranked_features_.dtype.kind in {"i", "f"}
    assert mic.ranked_features_.is_monotonic_increasing
...
def test_results_list(generate_dataset, dataset, model, metric):
    ...
    results = mic.fit_transform(X_train, X_test, y_train, y_test)

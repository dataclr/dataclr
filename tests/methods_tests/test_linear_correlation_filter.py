...
from dataclr.methods import LinearCorrelation
...
def test_ranked_features(dataset, model, metric):
    ...
    linear_correlation.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(linear_correlation.ranked_features_, pd.Series)
    assert not linear_correlation.ranked_features_.empty
    assert linear_correlation.ranked_features_.dtype.kind in {"i", "f"}
    assert linear_correlation.ranked_features_.is_monotonic_increasing
...
def test_results_list(dataset, model, metric):
    ...
    results = linear_correlation.fit_transform(X_train, X_test, y_train, y_test)
...
def test_empty_dataset():
    ...
    results = linear_correlation.fit_transform(

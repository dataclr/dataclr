...
from dataclr.methods import Kurtosis
...
def test_ranked_features(dataset, model, metric):
    ...
    kurtosis = Kurtosis(model=model, metric=metric, n_results=3, seed=42)
    kurtosis.fit_transform(X_train, X_test, y_train, y_test)
...
def test_results_list(dataset, model, metric):
    X_train, X_test, y_train, y_test = generate_dataset(dataset)
    kurtosis = Kurtosis(model=model, metric=metric, n_results=3, seed=42)
    results = kurtosis.fit_transform(X_train, X_test, y_train, y_test)
...
def test_empty_dataset():
    kurtosis = Kurtosis(model=LinearRegression(), metric="rmse", n_results=3, seed=42)
    results = kurtosis.fit_transform(

...
def test_ranked_features(dataset, model, metric):
    ...
    rfe.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(rfe.ranked_features_, pd.Series)
    assert not rfe.ranked_features_.empty
    assert rfe.ranked_features_.dtype.kind in {"i", "f"}
    assert rfe.ranked_features_.is_monotonic_increasing

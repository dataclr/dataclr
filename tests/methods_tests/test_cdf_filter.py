...
from dataclr.methods import CumulativeDistributionFunction
...
def test_ranked_features(generate_dataset, dataset, model, metric):
    ...
    cfd.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(cfd.ranked_features_, pd.Series)
    assert not cfd.ranked_features_.empty
    assert cfd.ranked_features_.dtype.kind in {"i", "f"}
    assert cfd.ranked_features_.is_monotonic_increasing
...
def test_results_list(generate_dataset, dataset, model, metric):
    ...
    results = cfd.fit_transform(X_train, X_test, y_train, y_test)

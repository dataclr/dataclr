...
from dataclr.methods import OptunaMethod
...
def test_ranked_features(dataset, model, metric):
    ...
    optuna = OptunaMethod(model=model, metric=metric, n_results=3, seed=42)
    optuna.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(optuna.ranked_features_, pd.Series)
    assert not optuna.ranked_features_.empty
    assert optuna.ranked_features_.dtype.kind in {"i", "f"}
    assert optuna.ranked_features_.is_monotonic_increasing
...
def test_results_list(dataset, model, metric):
    X_train, X_test, y_train, y_test = generate_dataset(dataset)
    optuna = OptunaMethod(model=model, metric=metric, n_results=3, seed=42)
    results = optuna.fit_transform(X_train, X_test, y_train, y_test)
...
def test_empty_dataset():
    optuna = OptunaMethod(model=LinearRegression(), metric="rmse", n_results=3, seed=42)
    results = optuna.fit_transform(

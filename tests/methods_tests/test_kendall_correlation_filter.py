...
from dataclr.methods import KendallCorrelation
...
def test_ranked_features(dataset, model, metric):
    ...
    kendall_correlation = KendallCorrelation(
        model=model, metric=metric, n_results=3, seed=42
    )
    kendall_correlation.fit_transform(X_train, X_test, y_train, y_test)

    assert isinstance(kendall_correlation.ranked_features_, pd.Series)
    assert not kendall_correlation.ranked_features_.empty
    assert kendall_correlation.ranked_features_.dtype.kind in {"i", "f"}
    assert kendall_correlation.ranked_features_.is_monotonic_increasing
...
def test_results_list(dataset, model, metric):
    ...
    results = kendall_correlation.fit_transform(X_train, X_test, y_train, y_test)
...
def test_empty_dataset():
    ...
    results = kendall_correlation.fit_transform(
